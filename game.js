...
import { AlgorandClient } from '@algorandfoundation/algokit-utils'; // Algokit Utils
...
class Wallet {
    ...
    __connectToPeraWallet() {
        return __awaiter(this, void 0, void 0, function* () {
            if (this.Connected == false) {
                //disconnect wallet session error catcher
                yield this.peraWallet.disconnect();
                try {
                    //create new connected session
                    const accounts = yield this.peraWallet.connect();
                    //this.peraWallet!.connector?.on('disconnect', this.handleDisconnectWallet);
                    this.accountAddress = accounts[0];
                    console.log("Account Address: ", this.accountAddress);
                    //fetch account info
    ...
    fetchWalletAssets() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // Get account asset info
            const accountAssets = yield this.indexerClient.lookupAccountAssets(this.accountAddress);
    ...
    handleDisconnectWallet(error, payload) {
    ...
    fetchAccountInfo() {
        return __awaiter(this, arguments, void 0, function* (accountAddress = this.accountAddress) {
            try {
                const accountInfo = yield this.indexerClient.lookupAccountByID(accountAddress).do();
...
class ThreeRender {
    ...
    LoadModel() {
        ...
        const DEBUG = false;
        loader.load('overworld_map.glb', (gltf) => {
            if (DEBUG) {
...
class Player extends GameObject {
    constructor() {
        ...
        */
        ...
        this.roll_direction = LittleJS.vec2(0, 1);
        this.StateBuffer = [];
        ...
        this.frameCounter = 0; // for timing frame changes to 1 sec or less
        this.mirror_ = false; //false
        this.animationCounter = 0.1; // 0.1 seconds
...
class UIObject extends EngineObject {
    constructor(localPos = uiDefaultPosition, size) {
        super();
        /**
...
class UIText extends UIObject {
    constructor(pos, size, text = '', align = 'center', font = LittleJS.fontDefault) {
        super(pos, size);
        this.pos = vec2();
        this.size = vec2();
        this.align = "center";
...
class UITile extends UIObject {
    constructor(pos, size, tileInfo, color = LittleJS.WHITE, angle = 0, mirror = false) {
        super(pos, size);
        this.pos = pos;
        this.size = size;
...
class UI extends UIObject {
    constructor() {
        ...
        this.Quit = null;
        //HUD Texture Buttons
        this.statsButton = null; // button triggered from input via stats() method
...
*/
    setDOMObjectVisibility(object, isVisible) {
        var _a, _b;
        // toggles the visibility of the carousel
        if (isVisible == true) {
            (_a = document.getElementById(object)) === null || _a === void 0 ? void 0 : _a.classList.remove("hidden"); // Show the carousel
        }
        else {
            (_b = document.getElementById(object)) === null || _b === void 0 ? void 0 : _b.classList.add("hidden"); // Hide the carousel
        }
    }
    //depreciated
    update() {
        ...
        if (!this.timer.elapsed() && this.timer.get() != 0 && this.SHOW_DIALOGUE == true) {
            ...
            const uiInfo = drawUIText(ipsum, vec2(250, 50), vec2(180, 40), LittleJS.WHITE, 8, LittleJS.BLACK, "center", "arial");
    ...
    createMenuOption(text, href, onPress) {
...
class OverWorld extends LittleJS.TileLayer {
    constructor() {
        ...
        this.layerCount = 0;
        //tileData: Array<any>;
        this.ground_layer = []; // matrix data type
        this.ENABLE = true;
        ...
        */
        ...
        this.ground_layer.forEach((row, y, array) => {
            row.forEach((column, x, array) => {
        ...
        trees_and_objects.forEach((row, y) => {
            row.forEach((val, x) => {
                val = parseInt(val, 10);
                if (val) {
                    //console.log("Val Debug: ", val); //works
    ...
    drawMapTile(pos, i = 80, layer, collision = 1) {
        const tileIndex = i;
